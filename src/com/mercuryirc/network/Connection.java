...
public class Connection implements Runnable {
	...
	public void ctcp(Entity target, String message) {
		...
		outputCallback.onCtcp(this, target, message);
	}
	...
	public void process(Entity target, String input) {
		if (input.startsWith("/")) {
			...
			if (input2.startsWith("query") && tokens.length > 1) {
				...
				outputCallback.onQuery(this, server.getUser(nick, true));
				...
				writeLine("PART " + target.getName());
			} else if (tokens[0].equals("j")) {
				writeLine("JOIN" + input.substring(1));
				...
				if (tokens.length == 3) {
					...
					outputCallback.onConnectionRequest(this, input.substring(start, start + tokens[2].length()), hostname, port, getLocalUser().getName());
				} else {
					inputCallback.onError(this, "Usage: /server hostname:port network_name");
					inputCallback.onError(this, "e.g. /server " + server.getHost() + ":" + server.getPort() + " " + server.getName());
				}
